...
function CreateLink(props) {
  ...
  function handleCreateLink() {
    if (!user) {
      ...
      const { description, resource } = values
      const newLink = {
        description,
        resource,
        postedBy: {
  ...
  return (
    <form onSubmit={handleSubmit} className=" flex flex-column mt3">
      <input
        ...
        name="description"
        placeholder="description for your tutorials"
        autoComplete="off"
        type="text"
      ...
      <input
        ...
        autoComplete="off"
        type="url"
        className={errors.resource && 'error-input'}
      />
      {errors.resource && <p className='error-text'> {errors.resource}</p>}
      <button
        className='button'
        type='submit'>
        Submit
